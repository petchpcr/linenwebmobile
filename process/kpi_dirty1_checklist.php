...
function load_question($conn, $DATA)
{
    ...
    $Sql = "SELECT IsStatus FROM kpi_dirty1 WHERE DocNo = '$DocNo'";
    $meQuery = mysqli_query($conn, $Sql);
    $Result = mysqli_fetch_assoc($meQuery);
    $return['IsStatus'] = $Result['IsStatus'];

    $Sql = "SELECT ID,Standard,Question FROM kpi_dirty1_question";
    ...
    while ($Result = mysqli_fetch_assoc($meQuery)) {
        ...
        $SqlD = "SELECT IsCheck FROM kpi_dirty1_detail WHERE DocNo = '$DocNo' AND Question_ID = '$ID'";
...
function save_checklist($conn, $DATA)
{
    ...
    foreach($RadioName as $key => $question_id) {
        ...
        $Sql = "SELECT COUNT(*) AS cnt FROM kpi_dirty1_detail WHERE DocNo = '$DocNo' AND Question_ID = $question_id";
        $meQuery = mysqli_query($conn, $Sql);
        $Result = mysqli_fetch_assoc($meQuery);
        $cnt = $Result['cnt'];
        ...
        if ($cnt == 0) { // ไม่ได้สร้างไว้
            $Sql = "INSERT INTO kpi_dirty1_detail(DocNo,Question_ID,IsCheck) VALUES('$DocNo',$question_id,$ChkResult[$key])";

        } else { // สร้างไว้แล้ว
            $Sql = "UPDATE kpi_dirty1_detail SET IsCheck = $ChkResult[$key] WHERE DocNo = '$DocNo' AND Question_ID = $question_id";
        }
    ...
    if ($have_null == 0) {
        $Sql = "UPDATE kpi_dirty1 SET IsStatus = 1 WHERE DocNo = '$DocNo'";
        mysqli_query($conn, $Sql);
    }
