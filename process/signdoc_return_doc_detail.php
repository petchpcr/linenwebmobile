...
function load_doc($conn, $DATA)
{
  ...
  $boolean = false;
  $boolean2 = false;
  $Sql = "SELECT RefDocNo,return_doc.IsStatus,
          DATE_FORMAT(return_doc.Modify_Date,'%d %M %Y') AS xdate,
          DATE_FORMAT(return_doc.Modify_Date,'%H:%i') AS xtime,
          Total,
          SignHospital,
          SignNH,
          ...
          FROM return_doc
          INNER JOIN users ON users.ID = return_doc.Modify_Code
          INNER JOIN site ON site.HptCode = return_doc.HptCode
  ...
  while ($Result = mysqli_fetch_assoc($meQuery)) {
    ...
    $return['SignHospital']  = $Result['SignHospital'];
  ...
  foreach ($ar_item as $key => $val) {
    ...
    $Sql = "SELECT SUM(Qty) AS Qty,SUM(Weight) AS Weight FROM return_detail WHERE DocNo = '$DocNo' AND ItemCode = '$val'";
