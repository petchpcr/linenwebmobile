<?php
    ...
    function create_claim($conn, $DATA){
        ...
        $Sql = "SELECT      department.HptCode,
                ...
                WHERE       repair.DocNo = '$repairDocNo'
        ...
        $Sql = "SELECT repair_detail.ItemCode,repair_detail.UnitCode,repair_detail.Weight,repair_detail.IsCheckList
                FROM repair_detail
                INNER JOIN repair ON repair_detail.DocNo = repair.DocNo 
                WHERE repair_detail.DocNo = '$repairDocNo'";
        $meQuery = mysqli_query($conn, $Sql);
        while ($Result = mysqli_fetch_assoc($meQuery)) {
            ...
            $Sql_qty = "SELECT Fail,Claim,Rewash FROM qccheckpass WHERE DocNo = '$repairDocNo' AND ItemCode = '$itemCode'";
            ...
            $Sql_check = "SELECT COUNT(RefDocNo) AS chkRewash,DocNo AS DocDetali FROM rewash WHERE RefDocNo = '$repairDocNo'";
            ...
            $Sql_check = "SELECT COUNT(RefDocNo) AS chkClaim,DocNo AS DocDetali FROM claim WHERE RefDocNo = '$repairDocNo'";
            $meQuery_check = mysqli_query($conn, $Sql_check);
            $Result_check = mysqli_fetch_assoc($meQuery_check);
            ...
            else if ($CheckList == 3 || $CheckList == 5 || $CheckList == 6 || $CheckList == 7) { // -------------- Rewash -------------- 
                // สร้างเอกสาร rewash
    ...
    function save_qc($conn, $DATA){
        ...
        $Sql = "SELECT COUNT(*) AS pNum FROM repair_detail WHERE DocNo= '$pDocNo' AND IsCheckList = 0";
        ...
        $Sql = "SELECT COUNT(*) AS itemNum FROM repair_detail WHERE DocNo= '$pDocNo'";
        ...
        if($pNum==$itemNum){ // IsCheckList = 1 QC pass all 
            ...
            while ($Result = mysqli_fetch_assoc($meQuery)) {
                ...
                $Sql_DocNo = "SELECT claim_detail.DocNo AS DocDetaliClaim
                                ...
                                INNER JOIN repair ON repair.DocNo = claim.RefDocNo 
                                WHERE repair.DocNo = '$pDocNo' 
                ...
                $Sql_DocNo = "SELECT rewash_detail.DocNo AS DocDetaliRewash
                                ...
                                INNER JOIN repair ON repair.DocNo = rewash.RefDocNo 
                                WHERE repair.DocNo = '$pDocNo' 
            ...
            $Sql = "UPDATE repair SET IsCheckList = 1,IsStatus = 4 WHERE DocNo= '$pDocNo'";

            
        }else{ // IsCheckList = 2 some send claim
            $Sql = "UPDATE repair SET IsCheckList = 2,IsStatus = 3 WHERE DocNo= '$pDocNo'";
        }
